...
import 'package:zamaan/features/task/data/repositories/measurement_unit_repository_impl.dart';
...
void main() {
  ...
  setUp(() {
    ...
    model = HiveMeasurementUnitModel.fromEntity(entity);
  });
  ...
  group('createEntity', () {
    test(
      ...
      when(() => mockDataSource.createEntity(
              newEntity: HiveMeasurementUnitModel.fromEntity(entity)))
  ...
  group('getEntity', () {
    ...
    test(
      ...
      expect(
          result,
          equals(const Left<Failure, MeasurementUnitEntity>(
              HiveFailure("Error"))));
      verify(() => mockDataSource.getEntity(id: model.id))
          .called(1); // Verify that get was only called once
      verifyNoMoreInteractions(mockDataSource);
