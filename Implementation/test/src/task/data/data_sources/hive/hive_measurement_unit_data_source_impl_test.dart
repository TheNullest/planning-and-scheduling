...
class MockHiveInit extends Mock
    implements HiveInitializer<HiveMeasurementUnitModel> {}

void main() {
  late HiveMeasurementUnitDataSourceImpl dataSource;
  late HiveInitializer<HiveMeasurementUnitModel> mockHiveInit;
  late HiveMeasurementUnitModel model;
  late List<String> keys;
  String boxName = HiveBoxConstants.MEASUREMENT_UNITS_BOX;
  setUp(() {
    mockHiveInit = MockHiveInit();
    dataSource = HiveMeasurementUnitDataSourceImpl(hiveBox: mockHiveInit);
    model = HiveMeasurementUnitModel.empty();
    keys = [
      uuidGenerator(),
      uuidGenerator(),
      uuidGenerator(),
  ...
  group('measurementUnitDataSource', () {
    ...
    test(
        ...
        () async {
      when(() => mockHiveInit.operator<List<HiveMeasurementUnitModel>>(
          job: any(named: 'job'),
      ...
      expect(result.isRight(), true);
      expect(result,
          equals(const Right<Failure, List<HiveMeasurementUnitModel>>([])));
      verify(() => mockHiveInit.operator<List<HiveMeasurementUnitModel>>(
          job: any(named: 'job'),
          boxName: boxName)).called(1); // Verify that get was only called once
      verifyNoMoreInteractions(mockHiveInit);
    });
    ...
    test(
      ...
      expect(
          result,
          equals(const Left<Failure, List<HiveMeasurementUnitModel>>(
              HiveFailure('Error'))));
      verify(() => mockHiveInit.operator<List<HiveMeasurementUnitModel>>(
          job: any(named: 'job'),
          boxName: boxName)).called(1); // Verify that get was only called once
    ...
    test(
        '[measurementUnitDataSource.getEntity] should retrieve entity by id from the box and returns [Right(HiveMeasurementUnitModel)] data',
        () async {
      when(() => mockHiveInit.operator<HiveMeasurementUnitModel>(
          job: any(named: 'job'),
      ...
      expect(result.isRight(), true);
      expect(result, equals(Right<Failure, HiveMeasurementUnitModel>(model)));
      verify(
        () => mockHiveInit.operator<HiveMeasurementUnitModel>(
            job: any(named: 'job'), boxName: boxName),
    ...
    test(
      ...
      expect(
          result,
          equals(const Left<Failure, HiveMeasurementUnitModel>(
              HiveFailure("Error"))));
      verify(
        () => mockHiveInit.operator<HiveMeasurementUnitModel>(
            job: any(named: 'job'), boxName: boxName),
